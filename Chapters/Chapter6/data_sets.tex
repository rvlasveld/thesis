...
The graphs display each from a single recording the inertial sensor values (see \Cref{tab:recorded_metrics} for an explanation of the metrics), with manually annotated activities above the data.
The manually and discovered change points are compared to draw quality conclusions over the algorithm's performance.
All of the used data sets, including the video material and manual labeling, are made available to the public as the \glsfirst{almende-data} \cite{vlasveld2014acras} for further research.
