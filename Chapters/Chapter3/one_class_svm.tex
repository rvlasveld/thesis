...
Consider a data set $\Omega = \{ (x_1, y_1),\allowbreak (x_2, y_2), \dots , (x_n, y_n) \}$; points $x_i \in \mathbb{R}^d$ in a (for instance two-dimensional) space where $x_i$ is the $\mathcal{I}$-th input data point and $y_i \in \{-1, 1\}$ is the $\mathcal{I}$-th output pattern, indicating the class membership.
...
The kernel function $K$ can have different forms, such as linear, polynomial and sigmoidal but the most used (and flexible) form is the Gaussian \gls{rbf}, in which the feature space $\mathcal{F}$ is a Hilbert Space of infinite dimensions *** ref needed ***:
\begin{equation}
...
The method maps the data points from input space $\mathcal{I}$ to a feature space $\mathcal{F}$ (following classical \gls{svm}s).
