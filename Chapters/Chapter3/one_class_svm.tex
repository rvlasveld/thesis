...
With that problem only data examples from one class is available and the objective is to recognize new data points that are not part of that class.
...
The final part of this section discusses the influende of model parameters on the performance of \glspl{svm}
...
The original data points $x \in I$ are through $\phi(x)$ projected to the points $z \in F$.
In that space the linear hyperplane is constructed by weights $w$.
This results in a non-linear hyperplane $y$ in input space $I$.
...
\centering
  ...
  \caption[Kernel mapping]{The non-linear boundary in the input space $\mathcal{I}$ (left) is transformed to a linear boundary in the feature space $\mathcal{F}$ (right) by mapping the data points with the function $\phi$. The kernel trick uses a function $K$ which performs an implicit mapping. Image from Wikipedia.org}
...
\centering
  ...
  \caption[\gls{svm} and the separating hyperplane]{Illustration of the separating hyperplane of a \gls{svm}. Here $w$ is the normal vector for the separating hyperplane and the distance between the two margins is $\frac{2}{\lVert{w}\rVert}$. Image from Wikipedia.org}
...
\end{enumerate}
An effect of the first property, combined with the equality to sparse represenations, is that \glspl{svm} often have good results, even in the case of high dimensional data (also known as the curse of dimensionality) or limited training examples \cite{cherkassky2007learning}.
The second property is what enables powerful adaption for \glspl{svm} to learn non-linear decision boundaries.
...
In \Cref{subsec:svm_model_parameters} this (hyper)parameter for a \gls{svm} is further discussed.
...
The constructed decision function $\mathcal{F}$ resembles the function discussed in \Cref{subsec:svm}.
...
It relates the \gls{oc-svm} model constuction to outlier detection and eventually change detection, leading to finding a temporal segmentation of time series data.
