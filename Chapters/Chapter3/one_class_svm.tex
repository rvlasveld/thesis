...
\\ \TODO{Add explanation why to use dual, which is: because the problem formulation scales with the number of samples $n$ and not the dimesionality $d$ of the samples.
More in Section 9.3 of \cite{cherkassky2007learning}.} \\
...
\TODO{Add something about robustness, discusses in section 9.2 from \cite{cherkassky2007learning}}
...
\begin{equation}
  H(\vectorsym{x}, \vectorsym{x}') = \operatorname*{exp} \left\{ - \frac{| \vectorsym{x} - \vectorsym{x}'|^2}{\sigma^2} \right\}.
\end{equation}
...
The width parameter is equal for all basis functions and determined a priori.
...
``Learning from Data'' \cite{cherkassky2007learning}. 2007, 1547 (book).