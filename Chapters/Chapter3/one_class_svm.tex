...
Assume an underlying probability distribution $P$ and a data point $\vectorsym{x}$ drawn from this distibution.
The goal is to find a subset $S$ of the input space, such that the probability that $\vectorsym{x}$ lies outside of $S$ equals some predetermined value $\nu$ between $0$ and $1$ \cite{scholkopf1999support}.
...
A \gls{svm} can create a boundary between linear-separable data points, making it a binary linear classifier.
...
\centering
  ...
  The weights of $\vectorsym{w}$ create a linear separating hypeplace, which maps the high dimensional vector to the predicted outcome $\vectorsym{y}$.}
